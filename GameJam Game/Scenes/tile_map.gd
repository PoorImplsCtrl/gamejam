...
func generate_grass():
	for y in range(ROWS):
		for x in range(COLS):
			var change = (x + y) % 2
			set_cell(grass_layer, Vector2i(x, y), tile_id, Vector2i(3 - change, 0))
...
func process_left_click(pos):
	...
	while not cells_to_reveal.is_empty():
		...
		if not is_number(cells_to_reveal[0]):
			cells_to_reveal = reveal_surrounding(cells_to_reveal, revealed_cells)
...
func reveal_surrounding(cells_to_reveal, revealed_cells):
...
func scan_mines(pos):
	...
	for i in get_all_surround_cells(pos):
	...
	if unflagged_mines == 0:
		for cell in reveal_surrounding([pos], []):
			if not is_mine(cell):
